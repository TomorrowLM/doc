...
可以设置宽高，除此之外，块级元素比较霸道，它独自占据一行高度（float浮动除外），一般可以作为其他容器使用，可容纳块级元素和行内元素。块级元素有以下特点：
...
- 行内元素不能设置宽高，和竖直方向的margin、padding 
...
**position:absolute和float**会让元素以display:inline-block的方式显示,可以设置长宽，默认宽度并不占满父元素。
...
clientY、clientX： 鼠标当前位置 相对于 视口顶部、浏览器可视区域左部 的位置；
...
e.target 是真正触发事件的目标元素   e.currentTarget  事件处理程序当前正在处理事件的那个元素 （绑定事件的元素）e.currentTarget始终等于this值。若绑定父元素，点击子元素冒泡触发事件，e.target指向子元素             
