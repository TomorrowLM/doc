...
还有一个因素也会引起css中px的变化，那就是**用户缩放**。例如，当用户把页面放大一倍，那么css中1px所代表的物理像素也会增加一倍；反之把页面缩小一倍，css中1px所代表的物理像素也会减少一倍。
...
```js
window.outerWidth/Height返回一个窗口的外部高度，包括所有界面元素（如工具栏/滚动条）
...
- Selection 与 execCommand API
  ...
  https://juejin.cn/post/6906635620752293902#heading-2、

- 图像写入剪切板

  https://juejin.cn/post/6909237803050074126#heading-9
