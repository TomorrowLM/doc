...
HTML常见的块级元素有： <div>、<table>、<form>、<p>、<ul>、
...
- https://blog.csdn.net/liwusen/article/details/80834546
  ...
  - 像素
    ...
    物理像素也称设备像素，只与设备或者说硬件有关，同样尺寸的屏幕，设备的密度越高，物理像素也就越多，所以**在不同的像素密度和相同的像素下，密度小的呈现的字体大**
...
- 外层盒子使用flex进行布局，不设置绝对宽度(使用rem或者%布局)，高度由里面的内容撑开。内层盒子使用rem或者%布局。
